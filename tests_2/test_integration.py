...
class TestIntegrationOldVsNew(unittest.TestCase):
    ...
    def test_simple_two_layer_setup(self):
        ...
        self.assertEqual(old_model.weak["nu"], new_system.weak_layer.nu, "Weak layer Poisson's ratio should be the same")
        self.assertEqual(old_model.weak["E"], new_system.weak_layer.E, "Weak layer Young's modulus should be the same")
        self.assertEqual(old_model.t, new_system.weak_layer.h, "Weak layer thickness should be the same")
        self.assertEqual(old_model.kn, new_system.weak_layer.kn, "Weak layer normal stiffness should be the same")
        self.assertEqual(old_model.kt, new_system.weak_layer.kt, "Weak layer shear stiffness should be the same")
        
        # Compare the Slab properties
        self.assertEqual(old_model.h, new_system.slab.H, "Slab thickness should be the same")
        self.assertEqual(old_model.zs, new_system.slab.z_cog, "Slab center of gravity should be the same")
        
        # Compare the Layer properties
        np.testing.assert_array_equal(old_model.slab[:, 0]*1e-12, new_system.slab.rhoi, "Layer density should be the same")
        np.testing.assert_array_equal(old_model.slab[:, 1], new_system.slab.hi, "Layer thickness should be the same")
        np.testing.assert_array_equal(old_model.slab[:, 2], new_system.slab.Ei, "Layer Young's modulus should be the same")
        np.testing.assert_array_equal(old_model.slab[:, 3], new_system.slab.Gi, "Layer shear modulus should be the same")
        np.testing.assert_array_equal(old_model.slab[:, 4], new_system.slab.nui, "Layer Poisson's ratio should be the same")
        
        # Compare all the attributes of the old and new model
        self.assertEqual(old_model.a, new_system.scenario.crack_l, "Crack length should be the same")

        # --- Compare results ---
        self.assertEqual(old_constants.shape, new_constants.shape, "Result arrays should have the same shape")
        ...
        np.testing.assert_allclose(old_constants, new_constants, rtol=1e-2, atol=1e-6, err_msg="Old and new implementations should produce very similar results")
    ...
    def test_simple_two_layer_setup_with_touchdown(self):
        ...
        self.assertEqual(old_model.tc, new_system.scenario.crack_h, "Crack height should be the same")
        self.assertEqual(old_model.a1, new_system.slab_touchdown.l_AB, "Transition length A should be the same")
        self.assertEqual(old_model.a2, new_system.slab_touchdown.l_BC, "Transition length B should be the same")
        self.assertEqual(old_model.td, new_system.slab_touchdown.touchdown_l, "Touchdown length should be the same")
        ...
        print(f"✓ Integration test with touchdown passed - implementations produce very similar results")
        print(f"  Result shape: {old_constants.shape}")
        print(f"  Max absolute difference: {max_abs_diff:.2e}")
        print(f"  Max relative difference: {max_rel_diff:.2e} ({max_rel_diff*100:.3f}%)")
