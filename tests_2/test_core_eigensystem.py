...
class TestEigensystemBasicProperties(unittest.TestCase):
    ...
    def test_eigensystem_initialization(self):
        ...
        self.assertIsNotNone(self.eigensystem.ewC, "Complex eigenvalues should be calculated")
        self.assertIsNotNone(self.eigensystem.ewR, "Real eigenvalues should be calculated")
        self.assertIsNotNone(self.eigensystem.evC, "Complex eigenvectors should be calculated")
        self.assertIsNotNone(self.eigensystem.evR, "Real eigenvectors should be calculated")
    ...
    def test_laminate_stiffness_parameters(self):
        ...
        self.assertGreater(self.eigensystem.A11, 0, "Extensional stiffness should be positive")
        self.assertGreater(self.eigensystem.D11, 0, "Bending stiffness should be positive")
        self.assertGreater(self.eigensystem.kA55, 0, "Shear stiffness should be positive")
...
class TestEigensystemEigenvalueCalculations(unittest.TestCase):
    ...
    def test_eigenvalue_classification(self):
        ...
        self.assertTrue(np.all(np.isreal(self.eigensystem.ewR)),
        ...
        if len(self.eigensystem.ewC) > 0:
            self.assertTrue(np.all(self.eigensystem.ewC.imag > 0),
                           "Complex eigenvalues should have positive imaginary parts")
    ...
    def test_eigenvector_dimensions(self):
        ...
        if len(self.eigensystem.ewR) > 0:
            self.assertEqual(self.eigensystem.evR.shape[0], 6,
                           "Real eigenvectors should be 6-dimensional")
            self.assertEqual(self.eigensystem.evR.shape[1], len(self.eigensystem.ewR),
                           "Number of real eigenvectors should match number of real eigenvalues")
            
        # Complex eigenvectors
        if len(self.eigensystem.ewC) > 0:
            self.assertEqual(self.eigensystem.evC.shape[0], 6,
                           "Complex eigenvectors should be 6-dimensional")
            self.assertEqual(self.eigensystem.evC.shape[1], len(self.eigensystem.ewC),
    ...
    def test_eigenvalue_shifts(self):
        ...
        self.assertEqual(len(self.eigensystem.sR), len(self.eigensystem.ewR),
                        "Real shifts should match real eigenvalues")
        self.assertEqual(len(self.eigensystem.sC), len(self.eigensystem.ewC),
                        "Complex shifts should match complex eigenvalues")
        
        # Shifts should be -1 or 0
        self.assertTrue(np.all(np.isin(self.eigensystem.sR, [-1, 0])),
                       "Real shifts should be -1 or 0")
        self.assertTrue(np.all(np.isin(self.eigensystem.sC, [-1, 0])),
...
class TestEigensystemSolutionMethods(unittest.TestCase):
    ...
    def test_complementary_solution_bedded(self):
        ...
        self.assertTrue(np.all(np.isreal(zh)), "Bedded complementary solution should be real")
    ...
    def test_complementary_solution_at_origin(self):
        ...
        self.assertTrue(np.isfinite(zh_bedded).all(), "Bedded solution should be finite at origin")
        self.assertTrue(np.isfinite(zh_free).all(), "Free solution should be finite at origin")
...
class TestEigensystemPhysicalConsistency(unittest.TestCase):
    ...
    def test_stiffness_scaling_with_properties(self):
        ...
        self.assertGreater(eig2.A11, eig1.A11, "Higher E should increase extensional stiffness")
        self.assertGreater(eig2.D11, eig1.D11, "Higher E should increase bending stiffness")
    ...
    def test_weak_layer_stiffness_influence(self):
        ...
        # Soft weak layer
        wl_soft = WeakLayer(rho=30, h=25, E=0.1)
        # Stiff weak layer
        wl_stiff = WeakLayer(rho=100, h=25, E=1.0)
        ...
        self.assertNotAlmostEqual(eig_soft.K[1, 0], eig_stiff.K[1, 0],
    ...
    def test_inclination_effect_on_loads(self):
        ...
        self.assertFalse(np.allclose(q_flat, q_inclined),
