...
class GeneralizedUnknownConstantsSolver:
    ...
    def solve_for_unknown_constants(  # pylint: disable=unused-argument
        ...
        nDOF_unsupported = 12
        logger.debug(
            "Number of supported segments: %s, DOF per supported segment: %s",
            nS_supported,
            nDOF_supported,
        )
        logger.debug(
            "Number of unsupported segments: %s, DOF per unsupported segment: %s",
            nS_unsupported,
            nDOF_unsupported,
        )
        ...
        Zh0_weak_layer = np.zeros(
            (
                nS_supported * (nDOF_supported - nDOF_unsupported),
                nS_supported * nDOF_supported + nS_unsupported * nDOF_unsupported,
            )
        )
        ...
        for i in range(nS):
            ...
            zhl = eigensystem.zh(x=0, length=length, has_foundation=has_foundation)
            zhr = eigensystem.zh(x=length, length=length, has_foundation=has_foundation)
            zpl = eigensystem.zp(
                x=0,
                phi=phi,
                theta=theta,
            ...
            if has_foundation:
                ...
                Zhi_weak_layer = cls._setup_conditions_weak_layer(
                    zl=eigensystem.zh(
                        x=0, length=length, has_foundation=has_foundation
                    ),
                    zr=eigensystem.zh(
                ...
                if pos in ("l", "left"):
                    Zh0_weak_layer[
                        0:6, global_start_slab : global_start_slab + nDOF_segment
                    ] = Zhi_weak_layer[0:6, :]
                    Zp0_weak_layer[0:6] += zpi_weak_layer[0:6]
                    if ki[i + 1]:  # Right neighboring segment is supported
                        Zh0_weak_layer[
                            6:18, global_start_slab : global_start_slab + nDOF_segment
                        ] = Zhi_weak_layer[6:, :]
                        ...
                        Zh0_weak_layer[
                            6:12, global_start_slab : global_start_slab + nDOF_segment
                        ] = Zhi_weak_layer[12:, :]
                    ...
                    Zh0_weak_layer[
                        global_start_weak_layer + local_start : global_start_weak_layer
                        + local_end,
                        global_start_slab : global_start_slab + nDOF_segment,
                    ] = np.concatenate(
                        [
                            Zhi_weak_layer[6 + local_start : 12, :],
                            Zhi_weak_layer[6 - local_end :, :],
                        ],
                        axis=0,
                    )
                    Zp0_weak_layer[
                        ...
                        [
                            zpi_weak_layer[6 + local_start : 12],
                            zpi_weak_layer[30 - local_end :],
                        ],
                        axis=0,
                    )
                    ...
                    Zh0_weak_layer[
                        local_start:,
                        global_start_slab : global_start_slab + nDOF_segment,
                    ] = Zhi_weak_layer[local_start:, :]
                    Zp0_weak_layer[local_start:] += zpi_weak_layer[local_start:]

            global_start_slab += nDOF_segment
            global_start_weak_layer += (
                (nDOF_supported - nDOF_unsupported) if has_foundation else 0
            )
