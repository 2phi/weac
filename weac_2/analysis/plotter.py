...
class Plotter:
    ...
    def __init__(
        ...
        if labels is None:
            ...
            if len(labels) != self.n_systems:
                raise ValueError(f"Number of labels ({len(labels)}) must match number of systems ({self.n_systems})")
            self.labels = labels
        ...
        if colors is None:
            ...
            if len(colors) != self.n_systems:
                raise ValueError(f"Number of colors ({len(colors)}) must match number of systems ({self.n_systems})")
            self.colors = colors
    ...
    def _get_systems_to_plot(
        ...
        if system_model is not None and system_models is not None:
            raise ValueError("Provide either 'system_model' or 'system_models', not both")
    ...
    def _get_labels_and_colors(self, systems_to_plot: List[SystemModel]) -> tuple[List[str], List[str]]:
    ...
    def plot_slab_profile(
        ...
        for i, (system, label, color) in enumerate(zip(systems_to_plot, labels, colors)):
            slab = system.slab
            
            # Calculate layer positions
            z_positions = np.concatenate([[0], np.cumsum([layer.h for layer in slab.layers])])
            densities = [layer.rho for layer in slab.layers]
            
            # Plot density profile
            for j, (z_start, z_end, rho) in enumerate(zip(z_positions[:-1], z_positions[1:], densities)):
                ax1.barh(z_start + (z_end-z_start)/2, rho, height=z_end-z_start, 
                        color=color, alpha=0.7, edgecolor='black', linewidth=0.5,
                        label=label if j == 0 else "")
            
            # Add weak layer
            wl_pos = slab.H + system.weak_layer.h/2
            ax1.barh(slab.H + system.weak_layer.h/2, system.weak_layer.rho, height=system.weak_layer.h,
                    color='red', alpha=0.8, edgecolor='black', linewidth=1,
                    hatch='///', label=f"Weak Layer ({label})" if i == 0 else "")
        ...
        for i, (system, label, color) in enumerate(zip(systems_to_plot, labels, colors)):
            ...
            z_positions = np.concatenate([[0], np.cumsum([layer.h for layer in slab.layers])])
            E_values = [layer.E for layer in slab.layers]
            
            # Append weak layer to z and E values
            z_positions = np.concatenate([z_positions, [slab.H + system.weak_layer.h]])
            E_values = np.concatenate([E_values, [system.weak_layer.E]])
            
            # Plot Young's modulus
            z_centers = [z_positions[j] + (z_positions[j+1]-z_positions[j])/2 for j in range(len(E_values))]
            ax2.plot(E_values, z_centers, 'o-', color=color, label=f"{label} (E)", markersize=6)

        ax2.set_xlabel('Young\'s Modulus (MPa)')
    ...
    def plot_deformed(
        ...
        if field in ['w', 'u']:
            ...
            if field == 'w':
                ...
                deformation = fq.u(z, h0=slab_height*500, unit='mm') * scale_factor / 1000
            
            # Plot original and deformed profiles
            ax.plot(x_m, np.zeros_like(x_m), 'k--', linewidth=1, alpha=0.5, label='Original')
            ax.plot(x_m, deformation, 'k-', linewidth=2, label=f'Deformed ({scale_factor}x)')
            ax.legend()
    ...
    def plot_stress_envelope(
        ...
        ax.scatter(sigma[0], tau[0], color='green', s=100, marker='o', label='Start', zorder=5)
        ax.scatter(sigma[-1], tau[-1], color='red', s=100, marker='s', label='End', zorder=5)
    ...
    def create_comparison_dashboard(
        ...
        for system, label, color in zip(system_models, labels, colors):
            ...
            for j, (z_start, z_end, rho) in enumerate(zip(z_positions[:-1], z_positions[1:], densities)):
                ax1.barh(z_start, rho, height=z_end-z_start, 
                        color=color, alpha=0.7, edgecolor='black', linewidth=0.5,
                        label=label if j == 0 else "")
        ...
        table_data = []
        headers = ['System', 'Slope (°)', 'Slab H (mm)', 'WL h (mm)', 'WL ρ (kg/m³)', 'Max |w| (mm)', 'Max |τ| (kPa)']
